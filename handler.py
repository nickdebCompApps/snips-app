...
def on_message(client, userdata, msg):
    ...
    if msg.topic == 'hermes/intent/searchWeatherForecast':
        ...
        volumeSetter = slots.get("volume_higher", None)
        if volumeSetter is not None:
            volumeSet = int(volumeSetter)
            state['volume'] = True
            state['volumeset'] = volumeSet
        state['volume'] = True
        state['volumeset'] = 'Up'
