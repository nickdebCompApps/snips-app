...
def printvars():
    ...
    global var_skip
    global var_quit
    print(var_pause, var_skip, var_quit)
    time.sleep(0.5)
...
class Player:
    ...
    def start(self):
        if isinstance(self.url, string_types):
            ...
            self.player = OMXPlayer(yurl, args=['-o','local'])
    ...
    def setVolume(self, volume):
        print(self.player.set_volume(volume))
                        ...
                        if self._play == True:
                            ...
                            if button_quit.is_pressed:
                                ...
                                if p.playlist == True:
                                    ...
                                    return False
                                time.sleep(1)
                            elif states['quit'] == True:
                                p.stop()
                                self._play = False
                                states['quit'] = False
                                return False
                            elif states['pause'] == True:
                                p.toggle()
                                time.sleep(1)
                                states['pause'] = False
                            elif states['volume'] == True:
                                if states['volumeset'] == 'Down':
                                    self.volume = p.volume()
                                    if self.volume >= 10:
                                        newVolume = int(self.volume) - 10
                                        self.volume = newVolume
                                    else:
                                        newVolume = int(self.volume)
                                        self.volume = newVolume
                                    states['volume'] = False
                                    states['volumeset'] = None
                                    p.setVolume(self.volume)
                                if states['volumeset'] == 'Up':
                                    self.volume = p.volume()
                                    if self.volume <= 90:
                                        newVolume = int(self.volume) + 10
                                        self.volume = newVolume
                                    ...
                                    states['volumeset'] = None
                                    p.setVolume(self.volume)
                                if isinstance(states['volumeset'], int):
                                    self.volume = p.volume()
                                    states['volume'] = False
                                    states['volumeset'] = None
                                    p.setVolume(states['volumeset'])
                            else:
                                time.sleep(0.1)
                                p.is_play()
                        else:
